...
Item {
  ...
  readonly property string displayIp: (currentIpv6 !== "n/a") ? currentIpv6 : currentIp
    ...
    onTriggered: function (action) {
      ...
      if (action === "copy") {
        if (displayIp && displayIp !== "n/a") {
          Quickshell.execDetached(["sh", "-c", `printf '%s' '${displayIp}' | wl-copy`]);
          ToastService.showNotice("IP copied to clipboard: " + displayIp);
