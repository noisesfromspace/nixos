...
Item {
  ...
  Item {
    ...
    property string currentIpv6: "n/a"
    ...
    property Process ipFetchProcess: Process {
      running: false
      command: ["curl", "-4", "-s", "-m", "10", "http://ip-api.com/json/?fields=status,continent,country,regionName,city,zip,lat,lon,timezone,currency,org,as,query"]
    ...
    // Process for fetching IPv6 address
    property Process ipv6FetchProcess: Process {
      running: false
      command: ["curl", "-6", "-s", "-m", "5", "https://ip.boers.email"]
      stdout: StdioCollector {
        id: ipv6StdoutCollector
      }

      onStarted: {
        Logger.d("IpMonitor", "Service fetching IPv6...");
      }

      onExited: function (exitCode, exitStatus) {
        var output = ipv6StdoutCollector.text.trim();
        Logger.d("IpMonitor", "Service IPv6 fetched:", output);
        if (exitCode === 0 && output.length > 0) {
          service.currentIpv6 = output;
        } else {
          Logger.d("IpMonitor", "Service IPv6 fetch failed (exitCode:", exitCode, ")");
          service.currentIpv6 = "n/a";
        }
      }
    }
    ...
    function fetchIp() {
      ...
      if (!service.ipv6FetchProcess) {
        Logger.e("IpMonitor", "Service ipv6FetchProcess is null!");
        return;
      }
      if (!service.ipv6FetchProcess.running) {
        Logger.d("IpMonitor", "Service starting ipv6FetchProcess");
        service.ipv6FetchProcess.running = true;
      }
    }
